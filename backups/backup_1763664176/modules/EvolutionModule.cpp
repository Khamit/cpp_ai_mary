...
{
    std::cout << "EvolutionModule initialized" << std::endl;
...
void EvolutionModule::evaluateFitness(const NeuralFieldSystem& system, double step_time) {
    // Сохраняем предыдущую приспособленность для сравнения
    double previous_fitness = current_metrics.overall_fitness;
    
    // 1. Награда за маленький размер кода
    current_metrics.code_size_score = calculateCodeSizeScore();
    
    // 2. Награда за скорость выполнения
    current_metrics.performance_score = calculatePerformanceScore(step_time);
    
    // 3. Награда за энергоэффективность  
    current_metrics.energy_score = calculateEnergyScore(system);
    
    // Общая приспособленность (все критерии одинаково важны)
    current_metrics.overall_fitness = 
        (current_metrics.code_size_score + 
         current_metrics.performance_score + 
         current_metrics.energy_score) / 3.0;
...
bool EvolutionModule::proposeMutation(NeuralFieldSystem& system) {
    ...
    // Предлагаем мутацию если приспособленность низкая
    if (current_metrics.overall_fitness < 0.7) {
        std::cout << "🔄 Low fitness (" << current_metrics.overall_fitness 
                  << "), proposing mutation..." << std::endl;
...
void EvolutionModule::evolveCodeOptimization() {
    ...
    std::cout << "🧬 EVOLVING CODE OPTIMIZATION..." << std::endl;
