      ...
      configText(font)
{
    ...
    // Нижняя панель
    bottomPanel.setSize(sf::Vector2f(static_cast<float>(windowWidth), static_cast<float>(bottom_panel_height)));
    bottomPanel.setPosition(sf::Vector2f(0, static_cast<float>(windowHeight - bottom_panel_height))); // ИСПРАВЛЕНО
    
    // Текст для нижней панели
    resourceText.setFont(font);
    resourceText.setCharacterSize(12);
    resourceText.setFillColor(TEXT_INFO);
    resourceText.setPosition(sf::Vector2f(10.0f, static_cast<float>(windowHeight - bottom_panel_height + 10))); // ИСПРАВЛЕНО
    
    configText.setFont(font);
    configText.setCharacterSize(12);
    configText.setFillColor(TEXT_INFO);
    configText.setPosition(sf::Vector2f(static_cast<float>(windowWidth / 2), static_cast<float>(windowHeight - bottom_panel_height + 10))); // ИСПРАВЛЕНО
}
