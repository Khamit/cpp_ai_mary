...
int main() {
    ...
    // 🧪 ДОБАВЬТЕ ЭТУ СТРОКУ ДЛЯ ТЕСТИРОВАНИЯ
    ...
    while (window.isOpen()) {
        ...
        if (simulation_running) {
            ...
            // 🧪 ВРЕМЕННО: Принудительный вызов эволюции каждые 100 шагов
            if (step % 100 == 0 && !system_in_stasis) {
                std::cout << "🔍 DEBUG: Forcing evolution at step " << step << std::endl;
                evolution.proposeMutation(neuralSystem);
                
                // Принудительно вызываем оптимизацию кода
                static bool evolution_called = false;
                if (!evolution_called && step >= 200) {
                    // Вызываем тестовый метод для проверки
                    evolution.testEvolutionMethods();
