...
bool EvolutionModule::proposeMutation(NeuralFieldSystem& system) {
    ...
    // 🧪 ВРЕМЕННО: ВСЕГДА предлагаем мутацию для тестирования
    std::cout << "🔄 TEST MODE: Always proposing mutation at step " << total_steps << std::endl;
    ...
    if (immutable_core.requestPermission("system_mutation")) {
        ...
        // 🧪 ВРЕМЕННО: Сразу запускаем оптимизацию кода
        evolveCodeOptimization();
